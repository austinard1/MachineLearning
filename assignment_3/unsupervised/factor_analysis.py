...
def main(X_train_smart, X_test_smart, y_train_smart, y_test_smart, X_train_bank, X_test_bank, y_train_bank, y_test_bank, args):
    ...
    for num_clusters in num_clusters_list_smart:
        fact = FactorAnalysis(n_components=num_clusters, random_state=27)
        fact.fit(X_train_smart)
        transformed = fact.transform(X_train_smart)
        variance_sum = transformed.var(axis=0).sum()
        variance_list_smart.append(variance_sum)
    ...
    fact = FactorAnalysis(n_components=num_comps_smart, random_state=27)
    fact.fit(X_train_smart)
    transformed = fact.transform(X_train_smart)
    ...
    for num_clusters in num_clusters_list_bank:
        fact = FactorAnalysis(n_components=num_clusters, random_state=27)
        fact.fit(X_train_bank)
        transformed = fact.transform(X_train_bank)
        variance_sum = transformed.var(axis=0).sum()
        variance_list_bank.append(variance_sum)
    ...
    fact = FactorAnalysis(n_components=num_comps_bank, random_state=27)
    fact.fit(X_train_bank)
    transformed = fact.transform(X_train_bank)
    ...
    fig.suptitle('Factor Analysis Variance Analysis (Left: Smart Grid, Right: Bank Loan)', fontsize=14)
    ...
    ax[0].set(xlabel='# of FA components', ylabel = 'Sum of Variances')
    ...
    ax[1].bar(new_feature_list_smart + 0.5*w, new_var_smart, width=w, color='green', label='FA Features ({0})'.format(num_comps_smart))
    ...
    ax[2].set(xlabel='# of fact components', ylabel = 'Sum of Variances')
    ...
    ax[3].bar(new_feature_list_bank + 0.5*w, new_var_bank, width=w, color='green', label='FA Features ({0})'.format(num_comps_bank))
    ...
    print('FA fit time (smart): ' + str(fa_fit_time))
    smart_transformed = fa.transform(X_train_smart)
    smart_transformed_test = fa.transform(X_test_smart)
    ...
    boost_fit_time = time() - boost_fit_t
    print('Boosting FA fit time (smart): ' + str(boost_fit_time))
    boost_pred_t = time()
    boost_pred = boosting_learner.predict(smart_transformed)
    boost_pred_time = time() - boost_pred_t
    print('Boosting FA predict time (smart): ' + str(boost_pred_time))
    boost_score = cross_val_score(boosting_learner, smart_transformed, y_train_smart, cv=10)
    print('Boosting FA cross validation score (smart): ' + str(np.mean(boost_score)))

    fa = FactorAnalysis(n_components=num_comps_bank, random_state=27)
    fa_fit_t = time()
    fa.fit(X_train_bank)
    fa_fit_time = time() - fa_fit_t
    print('FA fit time (bank): ' + str(fa_fit_time))
    bank_transformed = fa.transform(X_train_bank)
    bank_transformed_test = fa.transform(X_test_bank)
    ...
    boost_fit_time = time() - boost_fit_t
    print('Boosting FA fit time (bank): ' + str(boost_fit_time))
    boost_pred_t = time()
    boost_pred = boosting_learner.predict(bank_transformed)
    boost_pred_time = time() - boost_pred_t
    print('Boosting FA predict time (bank): ' + str(boost_pred_time))
    boost_score = cross_val_score(boosting_learner, bank_transformed, y_train_bank, cv=10)
    print('Boosting FA cross validation score (bank): ' + str(np.mean(boost_score)))
