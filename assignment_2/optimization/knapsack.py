...
def main(task):
    if 'tune_problem' in task:
        ...
        for max_weight_pct in max_weight_pct_list:
            ...
            experiment_name = 'knapsack_tuning_weight_pct_' + str(max_weight_pct)
            ...
            knapsack_tuning_fitness.append(knapsack_run_curves.loc[knapsack_run_curves['Fitness'].idxmax()]['Fitness'])
            knapsack_tuning_time.append(knapsack_run_curves.loc[knapsack_run_curves['Time'].idxmax()]['Time'])
            knapsack_tuning_fevals.append(2 * knapsack_run_curves.loc[knapsack_run_curves['Iteration'].idxmax()]['Iteration'])
        ...
        fig.suptitle('Knapsack Tuning w/ Simulated Annealing Optimizer', fontsize=14)
        ax[0].scatter(max_weight_pct_list, knapsack_tuning_fitness, c='r', marker='x', s=10)
        ax[0].set(xlabel='Max Weight %', ylabel = 'Max Fitness')
        ax[1].scatter(max_weight_pct_list, knapsack_tuning_time, c='g', marker='o', s=10)
        ax[1].set(xlabel='Max Weight %', ylabel = 'Max Runtime (s)')
        ax[2].scatter(max_weight_pct_list, knapsack_tuning_fevals, c='b', marker='+')
    ...
    # FOUR PEAKS GOOD FOR GENETIC
        ...
        problem = mlrose.DiscreteOpt(problem_size, knapsack_fitness, maximize=True, max_val=2)
        ...
        for population_size in population_sizes_list:
            experiment_name = 'ga_knapsack_tuning_population_size_' + str(problem_size)
        ...
        population_sizes_list= np.arange(10, 500, 10)
        for population_size in population_sizes_list:
            experiment_name = 'mimic_knapsack_tuning_population_size_' + str(problem_size)
        ...
        fig.suptitle('Knapsack Algorithm Tuning, problem size = ' + str(problem_size))
    ...
    if 'complexity_graph' in task:
        ...
        for problem_size in problem_size_list:
            ...
            knapsack_fitness = mlrose.Knapsack(weights, values, max_weight_pct)
            best_fitness_list = []
            problem = mlrose.DiscreteOpt(int(problem_size), knapsack_fitness, maximize=True, max_val=2)

            # RHC
            experiment_name = 'rhc_knapsack_complexity_size_' + str(problem_size)
            restart_list = [100]
            rhc = runners.RHCRunner(problem=problem,
                        experiment_name=experiment_name,
                        output_directory='knapsack',
            ...
            # SA
            experiment_name = 'sa_knapsack_complexity_size_' + str(problem_size)
            temperature_list = [2]
            sa = runners.SARunner(problem=problem,
                        experiment_name=experiment_name,
                        output_directory='knapsack',
            ...
            # GA
            experiment_name = 'ga_knapsack_complexity_size_' + str(problem_size)
            population_sizes_list=100,
            ...
            # MIMC
            experiment_name = 'mimic_knapsack_complexity_size_' + str(problem_size)
    ...
    if 'performance_graph' in task:
        ...
        # MIMC
        experiment_name = 'mimic_knapsack_performance_size_' + str(problem_size)
        population_sizes_list=200,
        ...
        #fig, ax = plt.subplots(2, 1, dpi=100, sharex=True, figsize=(5,4))
        fig, ax = plt.subplots(1,3,figsize=(12,3.5))
        fig.suptitle('Knapsack Algorithm Performance Analysis, problem size = ' + str(problem_size), fontsize=14)
        # ax[0].plot(problem_size_list, sa_fitness_list, 'b-', label='Simulated Annealing', linewidth=1)
        # ax[0].plot(problem_size_list, ga_fitness_list, 'g:', label='Genetic', linewidth=1)
        w = 1
        ...
        ax[0].plot(ga_iteration, ga_fitness, 'g-', label='Genetic', linewidth=2)
        ax[0].plot(mimic_iteration, mimic_fitness, '-.', color='orange', label='MIMIC', linewidth=2)
        ...
        ax[1].plot(mimic_time, mimic_fitness, '-.', color='orange', label='MIMIC', linewidth=2)
